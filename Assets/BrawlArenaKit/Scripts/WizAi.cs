...
public class WizAi : AIController {
	...
	public override void Update () {
		...
		//if health is low and heal ultimate is ready go into hiding
		...
		//if health is low or enemies are too close run to a safe point until health goes back up
		if (GetComponent<HealthController> ().Health < GetComponent<HealthController> ().MaxHealth / 3 || dist<MinDistance) {
			if(currentSafe==null)
		...
		if (dist < MaxDistance + 2 && FireRateDelay <= 0 && CurrentAmmo>=1) {
			...
			if (team != MainPlayer.team) {
				Gun.Fire (Gun.transform.rotation, 12,closestenemy.transform.position);
			} else {
				Gun.Fire (Gun.transform.rotation, 8,closestenemy.transform.position);
			}
